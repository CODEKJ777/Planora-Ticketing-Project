  ...
  return (
    <div className="min-h-screen p-6 max-w-5xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-3">
        <h1 className="text-4xl font-display font-bold text-white">Host Your Event</h1>
        <p className="text-slate-300">Create and manage your own events with Planora</p>
      </div>

      <div className="grid gap-6">
        <motion.div
          whileHover={{ scale: 1.02 }}
          className="bg-gradient-to-r from-violet-600/20 to-fuchsia-600/20 border-2 border-dashed border-violet-500/50 rounded-2xl p-12 text-center cursor-pointer hover:border-violet-500 transition"
          onClick={() => setIsOpen(true)}
        >
          <div className="space-y-3">
            <div className="text-5xl">🎪</div>
            <h2 className="text-2xl font-bold text-white">Create New Event</h2>
            <p className="text-slate-300">Click to start hosting your event</p>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex items-center justify-center p-6">
          <motion.div
            ...
            className="w-full max-w-2xl bg-slate-900 border border-white/10 rounded-2xl p-8"
          >
            ...
            <form onSubmit={submitForm} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500 mt-1 resize-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-slate-300">Registration Fee (₹) *</label>
                  <Input
                    type="number"
                    placeholder="1000"
                    value={form.price}
              ...
              <div>
                ...
                <div className="mt-1 border-2 border-dashed border-white/20 rounded-lg p-6 text-center hover:border-white/40 transition cursor-pointer">
                  <input
                    type="file"
                    accept="image/*"
