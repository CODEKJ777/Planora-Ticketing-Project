  ...
  useEffect(() => {
    ...
    async function fetchTicketAndEvent() {
      try {
        ...
        const ticketRes = await fetch(`/api/ticket/${ticketIdStr}`)
        if (ticketRes.ok) {
          const ticketData = await ticketRes.json()
          setTicket(ticketData)

          // Fetch event details if available
          if (ticketData.event_id || eventIdStr) {
            const eventsRes = await fetch('/api/events')
            if (eventsRes.ok) {
              const eventsData = await eventsRes.json()
              const foundEvent = eventsData.events?.find(
                (e: any) => e.id === (ticketData.event_id || eventIdStr)
              )
              if (foundEvent) {
                setEvent(foundEvent)
              }
    ...
    <>
      ...
      {!ticketIdStr ? (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 sm:p-6 md:p-8 flex items-center justify-center">
          <div className="max-w-md w-full bg-white/5 border border-white/10 rounded-2xl p-6 sm:p-8 text-center">
            <h1 className="text-white font-bold text-xl sm:text-2xl mb-2">Missing Ticket ID</h1>
            <p className="text-slate-300 text-sm sm:text-base mb-6">
              This page needs a valid ticket id in the URL.
            </p>
            <div className="flex items-center justify-center gap-3">
              <Link
                href="/"
                className="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-slate-700/50 hover:bg-slate-600/50 text-slate-200 font-semibold text-sm border border-slate-600/50"
              >
              ...
              <Link
                href="/my-tickets"
                className="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 text-white font-bold text-sm"
              >
                View My Tickets
              </Link>
            </div>
